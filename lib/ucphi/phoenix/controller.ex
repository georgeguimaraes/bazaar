defmodule Ucphi.Phoenix.Controller do
  @moduledoc """
  Phoenix controller that dispatches to your UCP handler.

  This controller is used internally by `Ucphi.Phoenix.Router`.
  ...
  def discovery(conn, _params) do
    handler = conn.assigns.ucphi_handler
  ...
  def create_checkout(conn, params) do
    ...
    case handler.create_checkout(params, conn) do
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        ...
        |> json(Ucphi.Errors.from_changeset(changeset))

      {:error, reason} ->
        conn
        |> put_status(:unprocessable_entity)
        |> json(Ucphi.Errors.from_reason(reason))
    end
  end
  ...
  def get_checkout(conn, %{"id" => id}) do
    ...
    case handler.get_checkout(id, conn) do
      ...
      {:error, :not_found} ->
        ...
        |> json(Ucphi.Errors.not_found("checkout_session", id))
  ...
  def get_order(conn, %{"id" => id}) do
    ...
    case handler.get_order(id, conn) do
      ...
      {:error, :not_found} ->
        conn
        |> put_status(:not_found)
        |> json(Ucphi.Errors.not_found("order", id))
