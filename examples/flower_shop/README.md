...
`FlowerShop.Handler` implements `Bazaar.Handler` and `bazaar_routes` mounts discovery, the catalog, carts, checkouts and orders for it. Bazaar's plugs handle version negotiation, idempotent replay and signature verification for platforms that sign, `Bazaar.Platform` finds the platform's webhook URL and `Bazaar.Webhook` delivers signed order events with a P-256 key generated at boot and published in the discovery profile. The app adds two routes the suite drives that aren't UCP: the simulate-shipping hook and `/healthz`.
