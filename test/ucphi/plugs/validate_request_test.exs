defmodule Ucphi.Plugs.ValidateRequestTest do
  ...
  describe "init/1" do
    test "uses default schemas when none provided" do
      ...
      assert opts[:create_checkout] == Ucphi.Schemas.CheckoutSession
      assert opts[:update_checkout] == Ucphi.Schemas.CheckoutSession
    end
  ...
  describe "call/2" do
    ...
    test "passes through when no schema for action", %{opts: opts} do
      ...
      refute conn.halted
      refute conn.assigns[:ucphi_validated]
    end
    ...
    test "validates and stores data on valid request", %{opts: opts} do
      ...
      refute conn.halted
      assert conn.assigns[:ucphi_validated] == true
      assert conn.assigns[:ucphi_data].name == "Test"
      assert conn.assigns[:ucphi_data].amount == 100
    end
    ...
    test "validates with checkout schema", %{opts: _opts} do
      ...
      refute conn.halted
      assert conn.assigns[:ucphi_validated] == true
      assert conn.assigns[:ucphi_data].currency == "USD"
    end
