defmodule Ucphi.Phoenix.RouterTest do
  ...
  defmodule LimitedRouter do
    ...
    scope "/" do
      pipe_through(:api)
      ucphi_routes("/api/v1", CheckoutOnlyHandler, only: [:checkout], webhooks: false)
    end
  end
  ...
  defmodule NoDiscoveryRouter do
    ...
    scope "/" do
      pipe_through(:api)
      ucphi_routes("/", FullHandler, discovery: false)
    end
  end
  ...
  describe "route generation" do
    test "generates discovery route" do
      ...
      assert discovery_route.plug == Ucphi.Phoenix.Controller
  ...
  describe "controller actions" do
    ...
    test "discovery returns profile", %{conn: conn} do
      conn =
        ...
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.discovery(%{})
    ...
    test "create_checkout returns 201 on success", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.create_checkout(%{"currency" => "USD"})
    ...
    test "get_checkout returns checkout on success", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.get_checkout(%{"id" => "found"})
    ...
    test "get_checkout returns 404 when not found", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.get_checkout(%{"id" => "not_found"})
    ...
    test "update_checkout returns updated checkout", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.update_checkout(%{"id" => "found", "total" => "99.99"})
    ...
    test "update_checkout returns 404 when not found", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.update_checkout(%{"id" => "not_found", "total" => "99.99"})
    ...
    test "cancel_checkout returns cancelled checkout", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.cancel_checkout(%{"id" => "found"})
    ...
    test "get_order returns order on success", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.get_order(%{"id" => "found"})
    ...
    test "get_order returns 404 when not found", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.get_order(%{"id" => "not_found"})
    ...
    test "cancel_order returns cancelled order", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.cancel_order(%{"id" => "found"})
    ...
    test "link_identity returns result on success", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.link_identity(%{"token" => "abc123"})
    ...
    test "link_identity returns error on failure", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.link_identity(%{})
    ...
    test "webhook returns success on valid webhook", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.webhook(%{"event" => "order.created"})
    ...
    test "webhook returns error on invalid webhook", %{conn: conn} do
      conn =
        conn
        |> Plug.Conn.assign(:ucphi_handler, FullHandler)
        |> Ucphi.Phoenix.Controller.webhook(%{})
