defmodule Bazaar.Plugs.IdempotencyTest do
  ...
  describe "call/2 without idempotency key" do
    test "passes through without idempotency key" do
      ...
      refute conn.halted
      refute conn.assigns[:bazaar_idempotency_key]
    end
  ...
  describe "call/2 with idempotency key (first request)" do
    test "sets idempotency assigns for new request" do
      ...
      refute conn.halted
      assert conn.assigns[:bazaar_idempotency_key] == "POST:/checkout-sessions:test-key-123"
      assert conn.assigns[:bazaar_idempotency_cache] == ETSCache
      assert conn.assigns[:bazaar_idempotency_ttl] == 86_400
    end
  ...
  describe "cache key building" do
    test "includes method in cache key" do
      ...
      refute post_conn.assigns[:bazaar_idempotency_key] ==
               patch_conn.assigns[:bazaar_idempotency_key]
    end
    ...
    test "includes path in cache key" do
      ...
      refute conn1.assigns[:bazaar_idempotency_key] == conn2.assigns[:bazaar_idempotency_key]
    end
  end
  ...
  describe "custom header name" do
    test "uses custom idempotency header" do
      ...
      refute conn.halted
      assert conn.assigns[:bazaar_idempotency_key] =~ "custom-key"
